...
import { GET_INVOICES, ADD_NEW_INVOICE, DELETE_INVOICE, UPDATE_INVOICE, ADD_NEW_PAYMENT } from "./actionType";
...
function* getInvoices() {
  try {
    ...
    yield put(invoiceApiResponseSuccess(GET_INVOICES, response.data, response));
  } catch (error) {
...
function* onAddNewInvoice({ payload: invoice }) {
  try {
    ...
    window.location.href = `/apps-invoices-details?id=${response.data.Orders._id}`;
    toast.success("Invoice Added Successfully", { autoClose: 3000 });
  } catch (error) {
    yield put(addInvoiceFail(error));
    toast.error("Failed to add Invoice", { autoClose: 3000 });
  }
}

function* onAddNewPayment({ payload: invoice }) {
  try {
    const response = yield call(addNewPayment, invoice);
    yield put(addInvoiceSuccess(response));
    console.log(response);
    toast.success("Payment Added Successfully", { autoClose: 3000 });
  } catch (error) {
    yield put(addInvoiceFail(error));
    toast.error("Failed to add Payment", { autoClose: 3000 });
  }
}
