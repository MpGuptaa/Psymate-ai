...
export const postFakeLogin = async (data) => {
  try {
    return await api.create(url.POST_FAKE_LOGIN, data);
  } catch (err) {
...
export const postFakeProfile = (data, credential) => api.put(url.POST_EDIT_PROFILE + `/${data.credential}`, data.data);
...
// Calendar
// get Events
export const getEvents = (id) => api.get(`${url.GET_EVENTS}${id}`);
...
export const getUsers = (type, page, searchParamsString) =>
  api.get(url.GET_USERS + `?${searchParamsString}&page=${page || 1}`);

export const getPatient = () => api.get(url.GET_USERS);

// Ecommerce
// get Products
export const getProducts = (page, search, searchBy) =>
  api.get(`${url.GET_PRODUCTS}?page=${page || 1}&search=${search || ""}&searchBy=${searchBy || "displayName"}`);

export const getDatas = (collection, page, search, searchBy) =>
  api.get(
    `${url.GET_DATAS}/${collection}?page=${page || 1}&search=${search || ""}&searchBy=${searchBy || ""}&limit=10`,
  );

export const addNewData = (collection, data) => api.create(`${url.GET_DATAS}/${collection}`, data);

export const updateData = (collection, search, searchBy, data) =>
  api.put(`${url.GET_DATAS}/${collection}?search=${search || ""}&searchBy=${searchBy || ""}`, data);

export const deleteData = (collection, search, searchBy) =>
  api.delete(`${url.GET_DATAS}/${collection}?search=${search || ""}&searchBy=${searchBy || ""}`);

export const getTools = (page, search, searchBy) =>
  api.get(`${url.GET_TOOLS}?page=${page || 1}&search=${search || ""}&searchBy=${searchBy || ""}&limit=10`);

export const addNewTool = (page, search, searchBy) =>
  api.get(`${url.GET_TOOLS}?page=${page || 1}&search=${search || ""}&searchBy=${searchBy || ""}&limit=10`);

export const updateTool = (page, search, searchBy) =>
  api.get(`${url.GET_TOOLS}?page=${page || 1}&search=${search || ""}&searchBy=${searchBy || ""}&limit=10`);

export const deleteTools = (page, search, searchBy) =>
  api.get(`${url.GET_TOOLS}?page=${page || 1}&search=${search || ""}&searchBy=${searchBy || ""}&limit=10`);

export const getAssessments = (page, search, searchBy) =>
  api.get(`${url.GET_ASSESSMENTS}?page=${page || 1}&search=${search || ""}&searchBy=${searchBy || ""}&limit=10`);

// get Products
export const getSalts = (page) => api.get(`${url.GET_SALTS}?page=${page}`);

// delete Product
export const deleteProducts = (product) => api.delete(url.DELETE_PRODUCT + "/" + product);

// delete Assessment
export const deleteAssessments = (product) => api.delete(url.DELETE_ASSESSMENT + "/" + product);

// delete Product
export const deleteSalts = (product) => api.delete(url.DELETE_SALT + "/" + product);
...
// add Assessment
export const addNewAssessment = (product) => {
  api.create(url.ADD_NEW_ASSESSMENT, product);
};
export const addNewSalt = (product) => api.create(url.ADD_NEW_SALT, product);

// update Products
export const updateProduct = (product) => {
  delete product._id;
  api.put(url.UPDATE_PRODUCT + "/" + product.id, product);
};

export const updateAssessment = (product) => {
  delete product._id;
  api.put(url.UPDATE_ASSESSMENT + "/" + product.query, product);
};
...
export const getCourse = (order) => api.get(url.GET_COURSES + `?search=${order.search}&searchBy=${order.searchBy}`);

// delete Product
export const deleteCourses = (product) => api.delete(url.DELETE_COURSE + "?id=" + product);

// add Products
export const addNewCourse = (product) => api.create(url.ADD_NEW_COURSE, product);

// update Products
export const updateCourse = (product) => api.put(url.UPDATE_COURSE + "?id=" + product.id, product);

export const getEstablishments = () => api.get(url.ESTABLISHMENTS);

export const getTemplates = () => api.get(url.GET_TEMPLATES);

// get Orders
export const getOrders = (type) => api.get(`${url.GET_ORDERS}?search=${type ? type : "appointment"}&searchBy=type`);
...
// get Manufacturers
export const getManufacturers = () => api.get(url.GET_MANUFACTURERS);

// Project
// get Project list
export const getProjectList = (searchKeyword, currentPage, projectId) => {
  let apiUrl;
  if (projectId) {
    apiUrl = `${url.GET_PROJECT_LIST}?projectId=${projectId}`;
  } else {
    apiUrl = searchKeyword
      ? `${url.GET_PROJECT_LIST}?keyword=${searchKeyword}&page=${currentPage}`
      : `${url.GET_PROJECT_LIST}?page=${currentPage}`;
  }
...
// Tasks
// get Task
export const getTaskList = (itemId, taskId, searchKeyword) => {
  let apiUrl;
  if (taskId) {
    apiUrl = `${url.GET_TASK_LIST}?taskId=${taskId}`;
  } else if (itemId) {
    apiUrl = `${url.GET_TASK_LIST}?itemId=${itemId}&Keyword=${searchKeyword}&searchBy=title`;
  } else {
    apiUrl = `${url.GET_TASK_LIST}?Keyword=${searchKeyword}&searchBy=title`;
  }
  return api.get(apiUrl);
};
// add Task
...
export const addNewPayment = (invoice) => api.create(url.ADD_NEW_PAYMENT, invoice);
...
// Prescription
//get Prescription
export const getPrescriptions = () => api.get(url.GET_PRESCRIPTIONS);

// add Prescription
export const addNewPrescription = (prescription) => api.create(url.ADD_NEW_PRESCRIPTION, prescription);

// update Prescription
export const updatePrescription = (prescription) =>
  api.update(url.UPDATE_PRESCRIPTION + "/" + prescription._id, prescription);

// delete Prescription
export const deletePrescription = (prescription) => api.delete(url.DELETE_PRESCRIPTION + "/" + prescription);
...
// Pages > Team
export const getTeamData = (team) => api.get(url.GET_TEAMDATA);
export const deleteTeamData = (team) => api.delete(url.DELETE_TEAMDATA, { credential: team });
...
export const addToCart = (user, med) => {
