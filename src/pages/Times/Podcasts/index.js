...
import { createArticles, createPodcast, updatePodcast } from "../apiTimes";
import axios from "axios";
import config from "../../../config";
const Podcast = () => {
  document.title = "Podcast | Psymate - Management Portal";
  ...
  useEffect(() => {
    if (id)
      axios.get(config.api.API_URL + `/podcast?id=${id}`).then((res) => {
  ...
  const handleCreateArticle = async () => {
    ...
    try {
      if (id) {
        const response = await updatePodcast(data, id)
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Create Podcast" pageTitle="Podcast" />
          <Row pb={5}>
            <Col lg={8} className="pb-5">
              <Card>
                <CardBody>
                  ...
                  <div className="mb-3 d-flex  flex-column">
                    <Label className="form-label">Audio Link </Label>

                    <Input
                      type="text"
                      placeholder="Enter the audio link"
                      value={audioLink}
                      onChange={handleAudioLink}
                    />
                  </div>
                </CardBody>
              </Card>
