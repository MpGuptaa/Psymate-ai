...
const VerticalLayout = (props) => {
  ...
  const getProfile = async () => {
    const response = await axios.get(
      config.api.API_URL +
        `/users?search=${userProfile._id}&searchBy=_id&exact=true`
    );
      dispatch(profileSuccess({ data: response?.data[0] }));

    setGetRoles(response?.data[0]?.roles);
  };
  useEffect(() => {
  ...
  return (
    <React.Fragment>
      ...
      {(navData || [])
        .filter((menuItem) => {
          return menuItem?.role?.some((role) => getRoles?.includes(role));
        })
        .map((item, key) => {
          return (
                  ...
                  >
                    <ul className="nav nav-sm flex-column test">
                      ...
                      {item.subItems &&
                        (item.subItems || [])
                          .filter((menuItem) => {
                            return menuItem?.role?.some((role) =>
